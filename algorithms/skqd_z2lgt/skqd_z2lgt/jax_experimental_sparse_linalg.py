...
This file is a copy of jax/experimental/sparse/linalg.py from JAX version 0.6.2 with updates by
yiiyama on the following points:
  - The matrix A is allowed to be Hermitian (and the vectors complex).
  - The matrix-application routine can take extra positional arguments.

The complete diff is found at the end of the file.
"""
...
def lobpcg_standard(
  ...
  return _lobpcg_standard_callable(A, X, m, tol, args=args, debug=False)
...
def _lobpcg_standard_callable(
  ...
  AX = A(X, *args)
  theta = jnp.sum(X.conjugate() * AX, axis=0, keepdims=True).real
  R = AX - theta * X

  def cond(state):
    i, _X, _P, _R, converged, _, _ = state
    return jnp.logical_and(i < m, converged < k)

  def body(state):
    i, X, P, R, _, theta, args = state
    ...
    theta, Q = _rayleigh_ritz_orth(A, XPR, *args)
    ...
    new_state = i + 1, X, P, R, converged, theta[jnp.newaxis, :k], args
    if debug:
...
def _check_inputs(A, X, *args):
  ...
  test_output = A(jnp.zeros((n, 1), dtype=X.dtype), *args)
...
def _project_out(basis, U):
  ...
  for _ in range(2):
    U -= _mm(basis, _mm(basis.conjugate().T, U))
    U = _orthonormalize(U)
...
def _rayleigh_ritz_orth(A, S, *args):
  ...
  SAS = _mm(S.conjugate().T, A(S, *args))
...
def _extend_basis(X, m):
  ...
  w = _mm(y, vt.conjugate().T * ((2 * (1 + s)) ** (-1/2))[jnp.newaxis, :])
  h = -2 * jnp.linalg.multi_dot(
      [w, w[k:, :].conjugate().T, other], precision=jax.lax.Precision.HIGHEST)
